...
if(train_saved):
    train_state = torch.load('/home/ge75tis/Desktop/oezyurt/model/6_1_UNET/tcc_gaussiandropout_35_epochs')
...
avg_val_loss_gridded = [[0 for i in range(5)] for j in range(5)]
...
def val():
    ...
    grid = [0.00001, 0.25, 0.5, 0.75, 0.99999]
    for k in range(5):
        for l in range(5):
            for i, (vals) in enumerate(val_loader):
                valid = vals.index_select(dim=1, index=input_indices[5]).to(device, dtype=torch.float32)
                target = vals.index_select(dim=1, index=out_index[5]).to(device, dtype=torch.float32)

                p = np.empty([len(valid), 6])
                for j in range(len(valid)):
                    p[j] = [grid[l], grid[l], grid[k], grid[l], grid[l], grid[l]]
